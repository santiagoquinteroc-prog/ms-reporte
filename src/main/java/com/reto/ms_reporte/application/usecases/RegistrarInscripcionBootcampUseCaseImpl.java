		...
		return reporteRepository.findByBootcampId(bootcampId)
				...
				.flatMap(reporte -> {
					if (reporte.getPersonas() != null && reporte.getPersonas().stream()
							.anyMatch(p -> p.getEmail() != null && p.getEmail().equalsIgnoreCase(persona.getEmail()))) {
						return Mono.error(new PersonaDuplicadaException("La persona con correo " + persona.getEmail() + " ya está inscrita en este bootcamp"));
					}
